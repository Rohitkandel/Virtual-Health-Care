...
try:
    ...
    print("Error: The model file 'models/svc.pkl' was not found.")
    svc = None
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        if item in symptoms_dict:
            ...
            closest_match, score = process.extractOne(item, symptoms_dict.keys())
            if score >= 80:  # Adjust this threshold as needed
                input_vector[symptoms_dict[closest_match]] = 1
                print(f"Assuming '{item}' means '{closest_match}'")
            else:
                unrecognized_symptoms.append(item)

    if unrecognized_symptoms:
        print(f"Warning: {', '.join(unrecognized_symptoms)} are not recognized symptoms.")
...
def home():
    if request.method == 'POST':
        ...
        if not symptoms or symptoms == "Symptoms":
            message = "Please enter your symptoms, separated by commas (e.g., itching, skin_rash, joint_pain)."
            return render_template('index.html', message=message)
        else:
            # Split and clean user's input
            ...
            try:
                ...
                message = f"Error: {str(e)} is not a recognized symptom. Please check your spelling."
                return render_template('index.html', message=message)
            except Exception as e:
